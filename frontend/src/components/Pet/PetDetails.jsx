...
export default function PetDetails() {
    ...
    return isLoading ? (
        ...
        <Card width={{ base: "100%", md: "70%" }} mb={{ base: 4, md: 0 }}>
            <CardBody>
            <Text fontSize={{ base: "2xl", md: "3xl" }} fontWeight="bold" textAlign="center" mb={4}>{titleCase(pet.name)}</Text>
                ...
                <Tbody>
                    ...
                    <Tr key={owner._id}>
                        ...
                        <Tooltip label="إزالة المالك" hasArrow>
                            <Button size="sm" variant="outline" colorScheme="red" onClick={() => handleRemovePetFromOwner(owner._id, pet._id)} rightIcon={<TbTrashXFilled />}>حذف</Button>
                        </Tooltip>
                        </Td>
                    </Tr>
                    ))}
            ...
            <Box mt={6} display="flex" flexDirection={{ base: "column", md: "row" }} gap={4} justifyContent="center">
                ...
                <Button onClick={() => navigate(`/pet-vaccination/${pet._id}`)} rightIcon={<IoMdMedical />}>كارت التطعيمات</Button>
                <Button onClick={() => navigate(`/pet-records/${pet._id}`)} rightIcon={<IoMdDocument />}>السجلات الصحية</Button>
        ...
        <Card width={{ base: "100%", md: "30%" }}>
            ...
            {owner !== null ? (
                ...
                <Box>
                <FormControl mb={4}>
                    <Input type="text" placeholder="رقم المحمول" value={ownerMobileNumber} onChange={(e) => setownerMobileNumber(e.target.value)} />
                </FormControl>
                <Button onClick={handleSearchOwner} width="100%" rightIcon={<IoMdSearch />}>بحث</Button>
                </Box>
            )}
            </CardBody>
        </Card>
